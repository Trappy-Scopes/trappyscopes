...
+ Logging refers to the list of events recorded by the computer system, python libraries, and the hardware. This logging is handled by the standard `logging` library in python. The logging level can be set in the `TrappyConfig`, which applies to the root logger.

+ When an experiment is started, a separate yaml file handler is added to the root logger. This redirects all logs, above the set level, to `logs.yaml...` in the experiment directory. Upon closure of the experiment, this handler is removed.


+ There is another kind of logging which happens during an experiment, which is the "events-logging". These are "records" that are relavent to the experiment and are generated by both the user and the eperimental procedures. For example, a typical record list would look like this:

```
t1: "protocol_initiated — Protocol X"
t2: "clock_started - UVC treatment"
t3: "user_note — Cells look synchronised."
```

+ This is usually achieved through:
```python
exp.record("protocol_initiated", name"Protocol X")
exp.record("clock_started", name="UVC treatment")
exp.record("user_note", name="Cells look synchronised.")
```

+ Other functions also call records. The following code is equivalent:
```python
from expframework.protocol import Protocol
protocol = Protocol("Protocol X")

exp.clocks = "UVC treatment"
exp.note("Cells look synchronised.")
```

+ Previous versions used the term "logs" for "records", for example a common idiom:
```python
exp.log("user_note", name="This is an older API and is now depreciated.")
```
